...
public class MapSettingsManager
{
    ...
    public RadarSettingsManager radarOptions;
    ...
    public void setRadarSettings(final RadarSettingsManager radarSettings) {
    ...
    public void saveAll() {
        final File settingsFileDir = new File(this.game.mcDataDir, "/mods/VoxelMods/");
        if (!settingsFileDir.exists()) {
            settingsFileDir.mkdirs();
        }
        this.settingsFile = new File(settingsFileDir, "voxelmap.properties");
        try {
            final PrintWriter out = new PrintWriter(new FileWriter(this.settingsFile));
    ...
    public String getOptionDisplayString(final int par1) {
        final int var2 = this.keyBindings[par1].getKeyCode();
