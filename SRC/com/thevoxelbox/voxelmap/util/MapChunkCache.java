...
public class MapChunkCache
{
    ...
    public MapChunkCache(final int width, final int height, final IMap minimap) {
    ...
    public void centerChunks(final int playerX, final int playerZ) {
        final Chunk currentChunk = Minecraft.getMinecraft().theWorld.getChunkFromBlockCoords(playerX, playerZ);
        if (currentChunk != this.lastPlayerChunk) {
            ...
            final int movedX = currentChunk.xPosition - this.lastPlayerChunk.xPosition;
            final int movedZ = currentChunk.zPosition - this.lastPlayerChunk.zPosition;
            if (Math.abs(movedX) < this.width && Math.abs(movedZ) < this.height && currentChunk.worldObj.equals(this.lastPlayerChunk.worldObj)) {
                this.moveX(movedX);
                this.moveZ(movedZ);
    ...
    public void fillAllChunks(final int playerX, final int playerZ) {
        final Chunk currentChunk = Minecraft.getMinecraft().theWorld.getChunkFromBlockCoords(playerX, playerZ);
        final int middleX = this.width / 2;
        final int middleZ = this.height / 2;
        for (int z = 0; z < this.height; ++z) {
    ...
    public void calculateChunks(final boolean realTimeUpdate) {
        if (!this.loaded) {
            return;
        }
