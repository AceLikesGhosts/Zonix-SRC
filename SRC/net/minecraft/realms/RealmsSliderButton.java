...
public class RealmsSliderButton extends RealmsButton
{
    ...
    public void renderBg(final int p_renderBg_1_, final int p_renderBg_2_) {
        if (this.getProxy().field_146125_m) {
            if (this.sliding) {
                this.value = (p_renderBg_1_ - (this.getProxy().x + 4)) / (float)(this.getProxy().func_146117_b() - 8);
                if (this.value < 0.0f) {
                    this.value = 0.0f;
                }
                if (this.value > 1.0f) {
            ...
            this.blit(this.getProxy().x + (int)(this.value * (this.getProxy().func_146117_b() - 8)), this.getProxy().y, 0, 66, 4, 20);
            this.blit(this.getProxy().x + (int)(this.value * (this.getProxy().func_146117_b() - 8)) + 4, this.getProxy().y, 196, 66, 4, 20);
        }
    }
    
    @Override
    public void clicked(final int p_clicked_1_, final int p_clicked_2_) {
        this.value = (p_clicked_1_ - (this.getProxy().x + 4)) / (float)(this.getProxy().func_146117_b() - 8);
        if (this.value < 0.0f) {
            this.value = 0.0f;
        }
        if (this.value > 1.0f) {
