...
public final class ZFontRenderer extends ZFont
{
    ...
    public ZFontRenderer(final ResourceLocation resourceLocation, final float size) {
        ...
        this.COLOR_CODE_START = '§';
        this.setupMinecraftColorCodes();
        this.setupBoldItalicIDs();
    }
    
    public float drawStringWithShadow(final String text, final double x, final double y, final int color, final int shadowColor) {
        final float shadowWidth = this.drawString(text, x + 1.0, y + 1.0, shadowColor, false);
    ...
    public float drawCenteredStringWithShadow(final String text, final float x, final float y, final int color) {
        this.drawString(text, x - this.getStringWidth(text) / 2 + 1.0, y + 1.0, color, true);
        return this.drawString(text, x - this.getStringWidth(text) / 2, y, color);
    }
            ...
            for (int i = 0; i < size; ++i) {
                ...
                if (character == this.COLOR_CODE_START && i < size) {
                    ...
                    if (colorIndex < 16) {
                        ...
                        final int cc = this.colorCode[colorIndex];
